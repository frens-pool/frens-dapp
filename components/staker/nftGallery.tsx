...
export const NftGallery = ({ poolAddress, isDepositing }) => {
    ...
    useEffect(() => {
        ...
        if (ethereum) {
            ...
            if (poolNftIds) {
                const poolIds: string[] = ((poolNftIds as BigNumber[]).map(x => x.toString()));
    ...
    const setPoolNftArray = async (FrensPoolShareContract: ethers.Contract, poolNftIDs: string[]) => {
        const poolNft = await Promise.all(
            poolNftIDs.map(async nftID => await jsonForNftId(FrensPoolShareContract, nftID))
        )
        setPoolNFTs(poolNft)
    }

    const getUserNfts = async (FrensPoolShareContract: ethers.Contract, poolNftIdsArray: string[]) => {
        let userNftIDs = await getUserNftIds(FrensPoolShareContract, accountAddress)
        const userPoolNfts = poolNftIdsArray.filter(id => userNftIDs.includes(id)) // intersection
        setUserNFTArray(FrensPoolShareContract, userPoolNfts)
    }

    const getUserNftIds = async (FrensPoolShareContract: ethers.Contract, ownerAddress) => {
    ...
    const jsonForNftId = async (FrensPoolShareContract: ethers.Contract, nftID: string) => {
    ...
    return (
        <div>
            ...
            <div className="text-center ">hard reload after staking required for now</div>
