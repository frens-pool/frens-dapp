...
export const StakeForm = ({ poolAddress, isDepositing, setIsDepositing }) => {
    ...
    const stakeAmount = ethInputForm > 0 ? ethInputForm.toString() : "0.000000000000001";

    const { isConnected } = useAccount();
    const { data:depositData, write:deposit, isError } = useDeposit({ address: poolAddress, val: stakeAmount });
        ...
        <form>
            <div>
                <div>
                    ...
                    {errors.ethInput &&
                        <div className="text-center text-red-500 font-medium my-2">
                            <span>greater 0.001 required</span>
                        </div>
                    }
                </div>
                    ...
                    { isConnected ?
                        <div className="flex flex-col justify-center">
                            <button
                                ...
                                className={`btn text-white ${isDepositing ? "btn-primary" : "bg-gradient-to-r from-pink-500 to-violet-500"}`} 
                                onClick={() => {
                                    deposit();
